...
static void rom_loading(void)
{
    if (rom_eof())
    {
        ...
        if (rom_FFFC && rom_FFFD)
            //main_run();
            printf("Loaded. Has reset vector.\n");
        else
    ...
    if (mbuf_len)
    {
        ...
        {
            rom_state = ROM_MIA_WRITING;
            mia_write_buf(rom_addr);
        }
    }
}
...
void rom_mon_install(const char *args, size_t len)
{
    ...
    bool is_rom = true;  //Handle .dsk and .tap files differently
...
void rom_task(void)
{
    ...
    case ROM_XRAM_WRITING:
        if (!rom_xram_writing())
            rom_state = ROM_LOADING;
        break;
    case ROM_MIA_WRITING:
        if (rom_action_is_finished())
        {
            rom_state = ROM_MIA_VERIFYING;
            mia_verify_buf(rom_addr);
        }
        break;
    case ROM_MIA_VERIFYING:
        if (rom_action_is_finished())
            rom_state = ROM_LOADING;
        break;
    }
