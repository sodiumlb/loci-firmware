...
void cpu_reclock(void)
{
    ...
    //if (ext_get(EXT_nRESET))
    if (ext_get(EXT_RESET))
...
void cpu_task(void)
{
    ...
    //if (cpu_run_requested && !ext_get(EXT_nRESET))
    if (cpu_run_requested && ext_get(EXT_RESET))
    {
...
int cpu_getchar(void)
{
    ...
    if (IOREGS(0x03A0) & 0b01000000)
    {
        IOREGS(0x03A0) &= ~0b01000000;
        int ch = IOREGS(0x03A2);
